...
public:
   ...
   * @param unit Character to specify unit for the coordinates. Acceptable
   * values:
  ...
  gaussianFunctionManager(const std::vector<std::string> densityMatFilenames,
                          ...
                          const char unit, const MPI_Comm &mpi_comm_parent,
   ...
   */
  void evaluateForQuad(const double *points, const double *weights,
                       const unsigned int nPoints, const bool evalBasis,
                       const bool evalBasisDerivatives,
                       const bool evalBasisDoubleDerivatives,
                       const bool evalSMat, const bool normalizeBasis,
   ...
   */
  void getRhoValue(const double *points, const unsigned int N,
                   const unsigned int spinIndex, double *rho) const;
   ...
   */
  void getRhoValue(const unsigned int quadIndex, const unsigned int spinIndex,
                   double *rho) const;
...
is
   ...
   */
  void getRhoGradient(const double *x, const int spinIndex,
                      std::vector<double> &returnValue);

  //    	  /**
  //    	   * @brief Get the value of the gradient of the density using a
  //    quadIndex.
  //    	   * This function should be used only after evaluateForQuad()
  //    is
  //     called,
  //    	   * so that the relevant basis data on the quadrature points
  //    that are
  //     part
  //    	   * of the quadIndex are pre-computed.
  //    	   *
  //    	   * @param quadIndex index of the quadrature grid on which to
  //    revaluate
  //    	   *  the density
  //    	   * @param spinIndex index of the spin (0 or 1)
  //    	   * @param[out] rhoGrad Pointer to gradient of the density at
  //    the
  //    	   *  quadrature points that are part of the quadIndex.
  //    	   *  rhoGrad is resized inside this function to appropriate
  //    size.
  //    	   *  The derivative with respect to j-th coordinate for the
  //    i-th point
   ...
   */
  double getBasisFunctionValue(const int basisId, const double *point) const;
   ...
   */
  double getBasisFunctionLaplacian(const int basisId,
                                   const double *point) const;
   ...
   */
  unsigned int getNumberBasisFunctions() const;

  const std::vector<double> &getSMat(const unsigned int quadIndex) const;

  const std::vector<double> &getDensityMat(const int spinIndex) const;

...
/**
 * @brief This class generates and stores adaptive finite element meshes for the
 * real-space dft problem.
 *
...
public:
  ...
  TriangulationManagerVxc(
      ...
      const dealii::parallel::distributed::Triangulation<3, 3>::Settings
          repartitionFlag = dealii::parallel::distributed::Triangulation<
              3, 3>::no_automatic_repartitioning);
  ...
  void generateParallelUnmovedMeshVxc(
      const std::vector<std::vector<double>> &atomPositions,
      dftfe::triangulationManager
          &dftTria); // TODO const triangulationManager will have been better
                     // but I dont think it is possible

  void generateParallelMovedMeshVxc(
  ...
  void computeMapBetweenParentAndChildMesh(
      ...
      std::vector<std::map<
          unsigned int, typename dealii::DoFHandler<3>::active_cell_iterator>>
   ...
   *
   */
  parallel::distributed::Triangulation<3> &getParallelMovedMeshVxc();

  /**
   * @brief returns constant reference to parallel unmoved triangulation
   *
   */
  parallel::distributed::Triangulation<3> &getParallelUnmovedMeshVxc();

  /**
   * @brief internal function which generates a coarse  vxc mesh which is
   * required for the load function call in restarts.
   *
   */

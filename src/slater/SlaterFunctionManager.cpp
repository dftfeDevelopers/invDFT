...
#define POLAR_ANGLE_TOL 1e-12
    namespace
    {
        ...
        double slaterRadialPartDerivative(const double r, const double alpha, const int n, const int derOrder)
        {
            if(derOrder == 0 && n >= 1)
        ...
        std::vector<double> getSphericalSlaterGradientAtOrigin(const int n,
                                                               const int l,
                                                               const int m,
                                                               const double alpha)
        {
                                                       ...
                                                       const double alpha)
        {
            ...
            else if(std::fabs(theta-0.0) < POLAR_ANGLE_TOL || std::fabs(theta-M_PI) < POLAR_ANGLE_TOL)
            {
        ...
        double getSphericalSlaterLaplacianAtOrigin(const int n,
                                                   const int l,
                                                   const int m,
                                                   const double alpha)
        {
        ...
        double getSphericalSlaterLaplacianAtPoles(const double r,
                                                  const double theta,
                                                  const int n,
                                                  const int l,
                                                  const int m,
        ...
        double getSphericalSlaterLaplacian(const std::vector<double> & x,
                                           const int n,
                                           const int l,
                                           const int m,
                                           const double alpha)
        ...
        namespace slaterFunctionManager
        {
                                  ...
                                  const int                 meshId)
            {
                ...
                QuadratureRuleManager::QuadratureNameId quadratureType = fieldQuadratureTypeManager.getFieldQuadratureType(fieldId);
                ...
                for (vtkIdType iElem = 0; iElem < numberElements; ++iElem)
                {
                    ...
                    const DoubleVector & elementQuadValuesCurrent = returnValue.getElementValues(iElem);
                    ...
                    for (int iQuadPoint = 0;
                        ...
                        for(int j = 0; j < numberComponents; ++j) {
                            ...
                            elementQuadValues[iQuadPoint*numberComponents + j] = pow(fVal,exponent);
    ...
    //
    double
    SlaterFunctionManager::getRhoValue(const double * x)
    {
    ...
    std::vector<double>
    SlaterFunctionManager::getRhoGradient(const double * x)
    {
        std::vector<double> returnValue(3,0.0);
        const int numBasis = d_basisFunctions.size();
        std::vector<double> basisFunctionValues(numBasis,0.0);
        std::vector<std::vector<double> > basisFunctionGradients(numBasis, std::vector<double>(3,0.0));
        for(unsigned int i = 0; i < numBasis; ++i)
        {
    ...
    double SlaterFunctionManager::getRhoLaplacian(const double * x)
    {
        ...
        for(unsigned int i = 0; i < numBasis; ++i)
        {
            ...
            for(unsigned int j = 0; j < numBasis; ++j)
            {
                ...
                const double term3 = 2.0*dotProduct(basisFunctionGradientI, basisFunctionGradientJ);
    ...
    //
    double
    SlaterFunctionManager::getBasisFunctionValue(const int basisId, const double * x)
    {
...
//    std::vector<double>
//    SlaterFunctionManager::getProjectedMO(QuadratureValuesContainer<DoubleVector> moIn,
//                                          const int meshId)
//    {
//#if defined(HAVE_MPI)
//
...
//        if(taskId == rootTaskId)
//        {
//            std::cout << "Diff Slater projection: " << diffL2Norm << std::endl;
//        }
//
//        return moCoeffs;
//    }
...
//    QuadratureValuesContainer<DoubleVector>
//    SlaterFunctionManager::computeMOSlaterFunction(const QuadratureValuesContainer<DoubleVector> & mo,
...
//        //
//
//        QuadratureRuleManager::QuadratureNameId quadratureType = fieldQuadratureTypeManager.getFieldQuadratureType(dft::ArrayNameManager::PSI);
//
//        //
//        // get handle to Adaptive quadrature rule container
//        //
...
//            //
//            const DoubleVector & elementQuadValuesCurrent = quadValues.getElementValues(iElem);
//            const DoubleVector & elementMOQuadValues = mo.getElementValues(iElem);
//
//            //
//            // copy current element quad values to a temporary storage
//            //
...
//                        quadratureRule.getGlobalPoint(iQuadPoint);
//
//                elementQuadValues[iQuadPoint] = elementMOQuadValues[iQuadPoint]*
...
//    QuadratureValuesContainer<DoubleVector>
//    SlaterFunctionManager::computeMOFromDensityCoeffs(const std::vector<double> & densityCoeffs,
//                                                      const int meshId)
//    {
//
//        //
...
//                                                     evaluateBasisValue(d_basisFunctions[i], &quadraturePoint[0]);
//
//                }
//            }
//

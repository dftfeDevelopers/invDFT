...
namespace invDFT
{
  namespace
  {
    ...
    //
    void
    vecAdd(const dftfe::distributedCPUVec<double> &x,
           dftfe::distributedCPUVec<double> &      y,
           const double                     a,
           const double                     b)
    {
    ...
    void
    vecScale(dftfe::distributedCPUVec<double> &x, const double a)
    {
      const unsigned int N = x.local_size();
      for (unsigned int i = 0; i < N; ++i)
        x.local_element(i) *= a;
  ...
  template <unsigned int FEOrder, unsigned int FEOrderElectro, dftfe::utils::MemorySpace memorySpace>
  void BFGSInverseDFTSolver<FEOrder,FEOrderElectro,memorySpace>::fnormCP(const std::vector<dftfe::distributedCPUVec<double>> &x,
                              ...
                              const BFGSInverseDFTSolver::LSType lsType)
  {
    ...
    template <unsigned int FEOrder, unsigned int FEOrderElectro, dftfe::utils::MemorySpace memorySpace>
    std::vector<dftfe::distributedCPUVec<double>> BFGSInverseDFTSolver<FEOrder,FEOrderElectro,memorySpace>::getSolution() const
  {
    return d_x;
  }

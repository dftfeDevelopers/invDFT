...
#define DFTFE_FINITE_DIFF_H 1e-3
  namespace
  {
    ...
    bool
    isNumber(double &i, std::string s)
    {
      char * end;
      double d;
      errno = 0;
    ...
    bool
    isInteger(int &i, std::string s, const int base = 10)
    {
      char *end;
      long  l;
      errno = 0;
    ...
    double
    getDistance(const double *x, const double *y)
    {
      double r = 0.0;
    ...
    double
    dotProduct(const std::vector<double> &x, const std::vector<double> &y)
    {
      ...
      if (numFDPoints == 3)
      ...
      else
        {
          const std::string message(
            "Invalid number of FD points. Please enter "
    ...
    std::vector<double>
    getNormConsts(const double *alpha, const int l, const int L)
    {
    ...
    double
    gfRadialPart(const double r, const int l, const double alpha)
    {
      return pow(r, l) * exp(-alpha * r * r);
    }

    double
    gfRadialPartDerivative(const double r,
                           const double alpha,
                           const int    l,
                           const int    derOrder)
    {
      if (derOrder == 0 && l >= 0)
        ...
        return (l * gfRadialPartDerivative(r, alpha, l - 1, derOrder - 1) -
                2 * alpha *
                  gfRadialPartDerivative(r, alpha, l + 1, derOrder - 1));
    }

    double
    getLimitingValueLaplacian(const int l, const int m, const double theta)
    {
      double returnValue = 0.0;
    ...
    double
    evaluateBasisValue(const invDFT::gaussianFunctionManager::basis *b,
                       const double *                               x)
    {
      const int     L = b->L;
      ...
      if (r < DFTFE_ZERO_RADIUS_TOL)
      ...
      else
        {
          ...
          for (unsigned int i = 0; i < 3; ++i)
            {
              returnValue[i] =
                C * (jacobianInverse[i][0] * partialDerivatives[0] +
          ...
          if (std::fabs(theta - 0.0) < DFTFE_POLAR_ANGLE_TOL ||
              std::fabs(theta - M_PI) < DFTFE_POLAR_ANGLE_TOL)
            {
              const double limitingVal =
                getLimitingValueLaplacian(l, modM, theta);
              const double term2 =
    ...
    double
    getBasisHigherOrderDerivativeFD(
      const double *                               point,
      const invDFT::gaussianFunctionManager::basis *basis,
          ...
          for (unsigned int iPoint = 0; iPoint < numFDPoints; ++iPoint)
            {
              ...
              returnValue +=
                factor * getBasisHigherOrderDerivativeFD(
                           &FDPoint[0], basis, indicesNext, numFDPoints, h);
            }
    ...
    void
    readAtomicCoordsAndBasisFileNames(
      ...
      while (std::getline(readFile, readLine))
        {
          AssertThrow(!readLine.empty(),
                      dealii::ExcMessage(
                        "Empty invalid found while reading atomic "
                        " coordinates in file " +
                        fileName));
          ...
          while (lineString >> word)
            {
              ...
              AssertThrow(!wordTrimmed.empty(),
                          dealii::ExcMessage(
                            "Empty column entry found while reading atomic "
                            "coordinates in file" +
                            fileName));
              if (count == 0)
                {
              ...
              if (count >= 1 && count <= 3)
                {
                  ...
                  if (isNumber(val, wordTrimmed))
                  ...
                  else
                    {
                      AssertThrow(false,
                                  dealii::ExcMessage(
                                    "Coordinate entry in file " + fileName +
                                    " is not a number"));
                    }
                }
          ...
          while (lineString >> word)
            {
              ...
              if (pos == std::string::npos)
                  ...
                  if (isInteger(nContracted, strNContracted))
                    {
                          ...
                          if (std::getline(readFile, readLine))
                            {
                              AssertThrow(
                                !readLine.empty(),
                                dealii::ExcMessage(
                                  "Empty line found in the Gaussian basis "
                                  " file " +
                                  fileName));
                              ...
                              AssertThrow(
                                ...
                                dealii::ExcMessage(
                                  "Exponent in the Gaussian basis file " +
                            ...
                            {
                              std::string msg =
                                "Undefined row for the contracted basis "
                      ...
                      for (unsigned int j = 0; j < numLChars; ++j)
                        {
                          invDFT::gaussianFunctionManager::contractedGaussian
                            *cg = new invDFT::gaussianFunctionManager::
                              contractedGaussian;
                          cg->L        = nContracted;
                ...
                {
                  std::string msg =
                    "Undefined L character(s) for the contracted "
                    " basis read in file " +
                    fileName;
                  AssertThrow(false, dealii::ExcMessage(msg));
                }
    ...
    void
    readMatrix(std::vector<std::vector<double>> &densityMat,
               const std::string                 fileName)
    {
      std::ifstream readFile;
      readFile.open(fileName.c_str());
    ...
    void
    convertCoordinatesFromAngsToBohr(std::vector<std::vector<double>> &coords)
    {
    ...
    void
    checkAtomicCoords(const std::vector<std::vector<double>> &coords1,
                      const std::vector<std::vector<double>> &coords2,
                      const double                            tol)
    {
      const int numAtoms = coords1.size();
      AssertThrow(coords1.size() == coords2.size(),
                  dealii::ExcMessage(
                    "The number atoms provided to the "
          ...
          for (unsigned int j = 0; j < numContractedGaussians; ++j)
            {
              ...
              if (lquantum == 'S' || lquantum == 's')
                ...
                {
                  std::string message(
                    "Invalid L character detected while reading in "
              ...
              // The ordering for cartesian orbitals are lexicographic - i.e.,
              // for p orbitals it's ordered as x,y,z. While in the spherical
              // form if one uses -l to +l ordering for the m quantum numbers
              // for l=1 (p orbital), then it translates to ordering the p
              // orbitals as y,z,x. To be consistent with QChem's ordering for p
              // orbital, we do it in an ad-hoc manner by ordering the m quantum
                  ...
                  for (unsigned int iM = 0; iM < 3; ++iM)
                    {
                      ...
                      for (unsigned int iContracted = 0;
                           iContracted < nContracted;
                           ++iContracted)
    ...
    void
    getBasisCutoff(const std::vector<invDFT::gaussianFunctionManager::basis *>
                                       &                  basisFunctions,
    ...
    double
    getClosestDistanceToBasis(const double *     points,
    ...
    void
    printData(const std::string filename,
              const std::vector<invDFT::gaussianFunctionManager::basis *>
                                                                  &                                     basisFunctions,
      ...
      for (unsigned int i = 0; i < numBasis; ++i)
        {
          ...
          outfile << "(l,m): (" << basisFunctions[i]->l << ","
                  << basisFunctions[i]->m << ")\t"
                  << "NormConsts: ";
          for (unsigned int j = 0; j < L; ++j)
            {
  ...
  {
    readAtomicCoordsAndBasisFileNames(d_atomicCoords,
                                      d_basisFileNames,
    ...
    storeBasisFunctions(d_basisFunctions,
                        d_contractedGaussians,
                        d_atomicCoords,
                        d_basisFileNames,
                        d_uniqueBasisFileNames);

    pcout << "Number of Gaussian basis: " << d_basisFunctions.size()
          << std::endl;
    pcout << "Density Matrix size: " << densityMatsTmp[0].size() << std::endl;
    AssertThrow(d_basisFunctions.size() == densityMatsTmp[0].size(),
                dealii::ExcMessage(
                  "Mismatch in number of gaussian basis and the size of "
    ...
    for (unsigned int i = 0; i < numAtoms; ++i)
      {
        ...
        const int numAtomicContractedGaussians =
          atomicContractedGaussians.size();
                ...
                for (unsigned int k = 0; k < 3; ++k)
                  {
                    for (unsigned int l = 0; l < 3; ++l)
                      {
                        basisDoubleDerivatives[i * nPoints * 9 + j * 9 + k * 3 +
                                               l] = dd[k * 3 + l];
                      }
                  }
              }
                ...
                for (unsigned int k = 0; k < nPoints; ++k)
                  {
                    overlap += d_basisVals[quadIndex][i * nPoints + k] *
                               d_basisVals[quadIndex][j * nPoints + k] *
                               weights[k];
                  }
    ...
    if (normalizeBasis)
      {
        ...
        for (unsigned int i = 0; i < numBasis; ++i)
          {
            const double factor =
              1.0 / sqrt(d_SMat[quadIndex][i * numBasis + i]);
                ...
                for (unsigned int iPoint = 0; iPoint < nPoints; ++iPoint)
                  {
                    for (unsigned int iDim = 0; iDim < 3; ++iDim)
                      {
                        d_basisDerivatives[quadIndex][i * nPoints * 3 +
                                                      iPoint * 3 + iDim] *=
                          factor;
                      }
                  }
...
//                    << " External val: " << SMatExternal[i][j] << std::endl;
  ...
  //
  void
  gaussianFunctionManager::getRhoValue(const unsigned int quadIndex,
                                       ...
                                       double *           rho) const
  {
    ...
    AssertThrow(it != d_basisVals.end(),
                dealii::ExcMessage(
                  "Gaussian basis values are not computed for the given "
                  "quadIndex"));
            ...
            for (unsigned int j = i + 1; j < numBasisWithCompactSupport; ++j)
              {
                ...
                val +=
                  2.0 *
                  d_densityMats[spinIndex][basisIdI * numBasis + basisIdJ] *
                  basisValI * basisValJ;
              }
          }
  ...
  //
  void
  gaussianFunctionManager::getRhoValue(const double *     points,
  ...
  //
  void
  gaussianFunctionManager::getRhoGradient(const double *x, const int
                                                               spinIndex,  std::vector<double> &returnValue)
  {
    ...
    std::vector<std::vector<double> > basisFunctionGradients(numBasis,
                                                            std::vector<double>(3,0.0)); 	  for(unsigned int i = 0; i < numBasis; ++i)
      {
            ...
            for(unsigned int k = 0; k < 3; ++k)
              {
                ...
                returnValue[k] += d_densityMats[spinIndex][i*numBasis +j]*(term1 + term2);
              }
  ...
  // const double term3 =
  // basisFunctionGradientI[iDim]*basisFunctionGradientJ[jDim]; 			  const
  // double
  // term4 = basisFunctionGradientI[jDim]*basisFunctionGradientJ[iDim];
  ...
  //		const double term3 = 2.0*dotProduct(basisFunctionGradientI,
  ...
  double
  gaussianFunctionManager::getBasisFunctionValue(const int     basisId,
  ...
  //
  std::vector<double>
  gaussianFunctionManager::getBasisFunctionDoubleDerivatives(
    const int     basisId,
    const double *x) const
  {
  ...
  {
    AssertThrow(spinIndex < d_numSpins,
                dealii::ExcMessage(
                  "Spin index passed to "
  ...
  unsigned int
  gaussianFunctionManager::getNumberBasisFunctions() const
  {

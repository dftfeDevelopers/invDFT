                        ...
                        // Read from params file for generality
{
  ...
  d_dftBaseClass = ((dftfe::dftClass<memorySpace> *)&dft);
...
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::
  ...
  if (d_inverseDFTParams.readGaussian) {
    ...
    if (d_inverseDFTParams.useLb94InInitialguess) {
      ...
      if (d_numSpins == 1) {
        ...
        std::fill(d_sigmaGradRhoTarget.begin(), d_sigmaGradRhoTarget.end(),
                  0.0);
  ...
  */
            ...
            //                  {
            //                    std::cout<<" Large value of
            //                    d_sigmaGradRhoTarget found at
            //                    "<<qpointCoord[0]<<"
            //                    "<<qpointCoord[1]<<"
            //                    "<<qpointCoord[2]<<"\n";
            //                  }
  ...
  if (d_inverseDFTParams.readSlater) {
    ...
    for (unsigned int iSpin = 0; iSpin < d_numSpins; iSpin++) {
      SlaterFunctionManager slaterFuncPrimaryObj(
  ...
  if (d_inverseDFTParams.readFEDensity) {
    ...
    pcout << " Time taken for reading density data from file  = "
          << readDensityEnd - readDensityStart << "\n";
  }
    ...
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int iQuad = 0; iQuad < numQuadraturePointsPerCellParent;
        ...
        if (d_rhoTarget[0][iElem * numQuadraturePointsPerCellParent + iQuad] <
          ...
          std::cout << " qPoint = ("
                    << d_quadCoordinatesParent[qPointCoordIndex + 0] << ","
  ...
  MPI_Allreduce(MPI_IN_PLACE, &diffInGaussianDensityL2Norm, 1, MPI_DOUBLE,
                MPI_SUM, d_mpiComm_domain);
  pcout << " Diff in gaussian density L2 norm  = "
        << diffInGaussianDensityL2Norm << "\n";

  MPI_Allreduce(MPI_IN_PLACE, &diffInGaussianDensityL1Norm, 1, MPI_DOUBLE,
                MPI_SUM, d_mpiComm_domain);
  pcout << " Diff in gaussian density L1 norm  = "
        << diffInGaussianDensityL1Norm << "\n";
...
void InverseDFTEngine<FEOrder, FEOrderElectro,
  ...
  */
  ...
  if (d_inverseDFTParams.useLb94InInitialguess) {
    ...
    xc_lda_vxc(&funcXLDA, totalOwnedCellsPsi * numQuadPointsPerPsiCell,
  ...
  for (unsigned int spinIndex = 0; spinIndex < d_numSpins; ++spinIndex) {
    ...
    for (unsigned int iQuad = 0;
         iQuad < totalLocallyOwnedCellsVxc * numQuadPointsPerCellInVxc;
         iQuad++) {
      initialPotValuesChildQuad[spinIndex].data()[iQuad] =
          initialPotValuesChildQuad[spinIndex].data()[iQuad] *
...
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::
  ...
  dftfe::poissonSolverProblem<FEOrderElectro> poissonSolverObj(mpiComm_domain);
...
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::
  ...
  for (unsigned int q_point = 0; q_point < totalQuadPtsAcrossAllProc;
       q_point++) {
...
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::
  ...
  quadPointCoordList.resize(numTotalQuadraturePointsParent,
                            std::vector<double>(3, 0.0));

  quadPointInterpolated.resize(numTotalQuadraturePointsParent);
  for (unsigned int iQuad = 0; iQuad < numTotalQuadraturePointsParent;
       iQuad++) {
  ...
  for (dealii::types::global_dof_index q_point = 0;
       q_point < totalQuadPtsAcrossAllProc; q_point++) {
    densityInputFile >> nodalValue;
    densityInputFile >> xcoordValue;
    densityInputFile >> ycoordValue;
    densityInputFile >> zcoordValue;
    ...
    if (isInputPointInsideBoundingBox) {
      ...
      if (closestPointIndices.size() >= 1) {
        ...
        if (distBetweenNodes < d_inverseDFTParams.distBetweenPoints) {
          ...
          densityParentQuad[0].data()[closestLocalPointId] = 0.5 * fieldValue0;
        }
      }
    }
  }
  ...
  for (dealii::types::global_dof_index iNode = 0; iNode < numberDofsChild;
    ...
    if (isInputPointInsideBoundingBox) {
      ...
      if (closestPointIndices.size() >= 1) {
        ...
        unsigned int closestGlobalPointId = coordIndex[closestLocalPointId];
        distBetweenNodes +=
...
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::readVxcInput() {
  ...
  pcout << " Time takne for reading data from file  = "
        << readVxcEnd - readVxcStart << "\n";
  d_targetPotValuesParentQuadData.resize(d_numSpins);
...
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::interpolateVxc() {
  ...
  projectVxcToParentMesh(vxcNodalParentMesh, d_vxcInitialChildNodes);
  ...
  if (d_inverseDFTParams.writeVtuFile) {
    ...
    data_out_vxc.write_vtu_with_pvtu_record("./", vtuFilename, 0,
                                            d_mpiComm_domain, 2, 4);
  }
  ...
  if (d_inverseDFTParams.writeToPoints) {
    ...
    dftfe::InterpolateCellWiseDataToPoints<dftfe::dataTypes::number,
                                           memorySpace>
...
template <unsigned int FEOrder, unsigned int FEOrderElectro,
          dftfe::utils::MemorySpace memorySpace>
void InverseDFTEngine<FEOrder, FEOrderElectro, memorySpace>::
    projectVxcToParentMesh(
        std::vector<dftfe::distributedCPUVec<double>>
            &vxcInitialGuessParentMesh,
        std::vector<dftfe::distributedCPUVec<double>> &vxcChildNodes) {

  std::vector<dftfe::utils::MemoryStorage<dftfe::dataTypes::number,
                                          dftfe::utils::MemorySpace::HOST>>
      vxcInterpolateToParent;
  ...
  for (unsigned int spinIndex = 0; spinIndex < d_numSpins; ++spinIndex) {
    ...
    d_inverseDftDoFManagerObjPtr->interpolateMesh2DataToMesh1QuadPoints(
        d_blasWrapperHost, vxcChildNodes[spinIndex], 1,
        fullFlattenedMapChildMemStorage, vxcInterpolateToParent[spinIndex], 1,
        1, 0, true);
    ...
    for (; cellPsi != endcPsi; ++cellPsi)
      if (cellPsi->is_locally_owned()) {
        ...
        for (unsigned int iQuad = 0; iQuad < numQuadPointsPerPsiCell; ++iQuad) {
          vxcOutputData[spinIndex][(iElemPsi * numQuadPointsPerPsiCell +
                                    iQuad)] =
              d_vxcLDAQuadData[spinIndex]
                              [(iElemPsi * numQuadPointsPerPsiCell + iQuad)] *
                  d_inverseDFTParams.factorForLDAVxc +
              vxcInterpolateToParent[spinIndex]
                  .data()[iElemPsi * numQuadPointsPerPsiCell + iQuad];
        }
        iElemPsi++;
      }
    ...
    d_dftBaseClass->l2ProjectionQuadToNodal(
        d_basisOperationsHost, *d_constraintDFTClass,
        d_dftDensityDoFHandlerIndex, d_dftQuadIndex, vxcOutputData[spinIndex],
        vxcInitialGuessParentMesh[spinIndex]);

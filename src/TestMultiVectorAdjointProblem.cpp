...
namespace dftfe
{
              ...
              const MPI_Comm &   interpoolcomm)
  {
    ...
    std::cout<<" matrixFreeQuadratureComponentRhsDensity = "<<matrixFreeQuadratureComponentRhs<<"\n";
    ...
    MultiVectorAdjointLinearSolverProblem<memorySpace> adjointProblemObj(mpi_comm_parent,
                                                        mpi_comm_domain);
    ...
    dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST> targetDensity;
    ...
    dealii::FEValues<3> fe_valuesRhs(dofHandlerAdjoint->get_fe(),
                                     quadratureRhs,
                                     dealii::update_JxW_values);


    unsigned int                   defaultBlockSize = 100;
    distributedCPUMultiVec<double> psiBlockVec, boundaryValues,
      multiVectorOutput;
        ...
        for (unsigned int kPoint = 0; kPoint < noKPoints; ++kPoint)
          {
            ...
            dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST> differenceInDensities;

            differenceInDensities.resize(numTotallyOwnedCells*numberQuadraturePointsRhs,0.0);
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  for (unsigned int iQuad = 0;
                       iQuad < numberQuadraturePointsRhs;
                       iQuad++)
                    {
                      // Assumes the ordering of writing is same


                      targetDensity[iElem*numberQuadraturePointsRhs + iQuad] = densityFromSCF[0][iElem*numberQuadraturePointsRhs + iQuad];

                      differenceInDensities[iElem*numberQuadraturePointsRhs + iQuad] =
                        0.1*targetDensity[iElem*numberQuadraturePointsRhs + iQuad];

                      densityDiffNorm += differenceInDensities[iElem*numberQuadraturePointsRhs + iQuad] *
                                         differenceInDensities[iElem*numberQuadraturePointsRhs + iQuad] *
            ...
            for (unsigned int jvec = 0; jvec < numberOfWaveFunctions;
                 jvec += defaultBlockSize)
              {
                ...
                for (unsigned int iNode = 0; iNode < locallyOwnedSize; ++iNode)
                  for (unsigned int iWave = 0; iWave < currentBlockSize;
                    ...
                    psiBlockVec.data()[iNode * currentBlockSize + iWave] =
                      waveCurrentKPoint[iNode * numberOfWaveFunctions + jvec +
                                        iWave];
                     ...
                     iBlock++)
                  {
                    shiftValues[iBlock] =
                      eigenValuesInput[kPoint]
                                      [numberOfWaveFunctions * spinIndex +
                                       iBlock + jvec];
                ...
                adjointProblemObj.updateInputPsi(psiBlockVec,
                ...
                linearSolver.solve(adjointProblemObj,
                                   BLASWrapperPtr,
                                   multiVectorOutput,
                                   boundaryValues,
                                   locallyOwnedSize,
                     ...
                     iBlock++)
                  {
                    vectorTools::createDealiiVector<double>(
                      matrixFreeData.get_vector_partitioner(
                        matrixFreePsiVectorComponent),
                      1,
                      singleVectorOutput[iBlock]);

                    for (unsigned int iNode = 0; iNode < locallyOwnedDofs;
                         iNode++)
                      {
                        singleVectorOutput[iBlock].local_element(iNode) =
                          multiVectorOutput
                            .data()[iNode * currentBlockSize + iBlock];
                      }
                    ...
                    data_out.add_data_vector(singleVectorOutput[iBlock],
                                             outputVecName);
                  }
                ...
                data_out.write_vtu_with_pvtu_record(
                  "./", "adjoint", kPointSpinIndex, mpi_comm_domain, 2, 4);
              }
          }
      }
  }

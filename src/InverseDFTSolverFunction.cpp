...
namespace invDFT
{
  ...
  void InverseDFTSolverFunction<FEOrder,FEOrderElectro,memorySpace>::reinit(
    ...
    std::vector<std::shared_ptr<
      dftfe::basis::FEBasisOperations<dftfe::dataTypes::number, double, memorySpace>>> &
      basisOperationsParentPtr,
    std::vector<std::shared_ptr<
      dftfe::basis::FEBasisOperations<dftfe::dataTypes::number, double, dftfe::utils::MemorySpace::HOST>>>
    ...
    const std::shared_ptr<dftfe::TransferDataBetweenMeshesIncompatiblePartitioning<memorySpace>>
    ...
    const inverseDFTParameters & inverseDFTParams)
  {
    ...
    dftfe::dftUtils::createBandParallelizationIndices(d_mpi_comm_interband,
                                               d_numEigenValues,
...
dftfe::distributedCPUVec<double> tempVec =
        dftfe::distributedCPUVec<double>(locally_owned_dofs_,
                                ghost_indices_,
                                d_mpi_comm_domain);
    ...
    for(dftfe::size_type i = 0 ; i < d_numLocallyOwnedCellsChild*numQuadraturePointsPerCellChild;i++)
      {
...
//      }
  ...
  void InverseDFTSolverFunction<FEOrder,FEOrderElectro,memorySpace>::writeVxcDataToFile(
    std::vector<dftfe::distributedCPUVec<double>> &pot,
    ...
    if (poolId == minPoolId && bandGroupId == minBandGroupId)
      {
        ...
        for (unsigned int i = 0; i < data.size(); ++i)
          ...
          dftfe::dftUtils::MPIWriteOnFile().writeData(dataRawPtrs,
                                             filename,
                                             d_mpi_comm_domain);
      }
    ...
    std::vector<double> &                   loss)
  {
    ...
    dftfe::computeRhoFromPSI<dftfe::dataTypes::number>(&eigenVectorsMemSpace,
    ...
    MPI_Barrier(d_mpi_comm_domain);
        ...
        d_transferDataPtr->interpolateMesh2DataToMesh1QuadPoints(
          d_BLASWrapperHostPtr,pot[iSpin], 1, d_fullFlattenedMapChild,d_potParentQuadDataForce[iSpin],d_resizeMemSpaceVecDuringInterpolation);
            #if defined(DFTFE_WITH_DEVICE)          
    if (memorySpace == dftfe::utils::MemorySpace::DEVICE)
      dftfe::utils::deviceSynchronize();
#endif
...
pcout<<"d_resizeMemSpaceBlockSizeChildQuad in inverse dft Solver func = "<<d_resizeMemSpaceBlockSizeChildQuad<<"\n";  
    ...
    for (unsigned int iSpin = 0; iSpin < d_numSpins; ++iSpin)
      {
        ...
        d_transferDataPtr->interpolateMesh2DataToMesh1QuadPoints(
         d_BLASWrapperHostPtr, pot[iSpin], 1, d_fullFlattenedMapChild,d_potParentQuadDataSolveEigen[iSpin],d_resizeMemSpaceVecDuringInterpolation);
        d_computingTimerStandard.leave_subsection(
          "interpolate child data to parent quad");
                ...
                for (unsigned int iEig = 0; iEig < d_numEigenValues; ++iEig)
                  {
                    ...
                    d_fractionalOccupancy[iKpoint][d_numEigenValues * iSpin +
                                                   iEig] =
                            dftfe::dftUtils::getPartialOccupancy(eigenValue,
                                                    fermiEnergy,
                                                    dftfe::C_kb,
                                                    d_dftParams->TVal);
  ...
  template <unsigned int FEOrder, unsigned int FEOrderElectro, dftfe::utils::MemorySpace memorySpace>
  void
  InverseDFTSolverFunction<FEOrder,FEOrderElectro, memorySpace>::dotProduct(const dftfe::distributedCPUVec<double> &vec1,
